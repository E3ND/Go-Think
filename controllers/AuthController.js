const User = require('../models/User')
const bcrypt = require('bcryptjs')


module.exports = class AuthController {
    static login(req, res) {
        res.render('auth/login')
    }

    static async loginPost(req, res) {
        const { email, password } = req.body

        // blank validation
        if(!email || !password) {
            req.flash('messageBlank', 'Não se pode enviar dados vazios, preencha todos os campos!')
            res.render('auth/login')

            return
        }

        const user = await User.findOne({
            where: {
                email: email
            }
        })

        // Find user
        if(!user) {
            req.flash('message', 'Usuário não encontrado!')
            res.render('auth/login')

            return
        }

        // Check if passwords match
        const passwordMatch = bcrypt.compareSync(password, user.password)

        if(!passwordMatch) {
            req.flash('message', 'Senha incorreta, tente novamente!')
            res.render('auth/login')

            return
        }

        // Inicializando a session
        req.session.userid = user.id

        req.flash('messageConfirm', 'Logado com sucesso!')
        
        req.session.save(() => {
            res.redirect('/')
        })
    }

    static register(req, res) {
        res.render('auth/register')
    }

    static async registerPost(req, res) {
        const { name, email, password, confirmpassword } = req.body

        // blank validation
        if(!name || !email || !password || !confirmpassword) {
            // Flash menssage
            req.flash('messageBlank', 'Não se pode enviar dados vazios, preencha todos os campos!')
            res.render('auth/register')

            return
        }

        // Password match validation
        if(password != confirmpassword) {
            // Flash menssage
            req.flash('messages', 'As senhas não conferem, tente novamente!')
            res.render('auth/register')

            return
        }

        // Check if user exists
        const checkIfUserExists = await User.findOne({
            where: {
                email: email
            }
        })

        if(checkIfUserExists) {
            req.flash('message', 'E-mail já está em uso!')
            res.render('auth/register')

            return
        }

        // Create a password
        // Criptografo a senha adcionando 10 caracteres e depos uso a criptografia do bcrypt
        const salt = bcrypt.genSaltSync(10)
        const hashedPassword = bcrypt.hashSync(password, salt)

        const user = {
            name,
            email,
            password: hashedPassword
        }

        // Inicializando a session
        try {
            const createdUser = await User.create(user)

            req.session.userid = createdUser.id

            req.flash('messageConfirm', 'Cadastrado com sucesso!')
            req.session.save(() => {
                res.redirect('/')
            })

        } catch(err) {
            console.log(err)
        }

    }

    static logout(req, res) {
        // Logout
        req.session.destroy()
        res.redirect('/login')
    }
}